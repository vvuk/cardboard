/* -*- Mode: javascript; js-indent-level: 4; indent-tabs-mode: t; tab-width: 4 -*- */
...
Crafty.c("DOM", {
	...
	*/
	...
	_cssStyles: null,
	// holds attached classes, other than the component classes
	_customClasses: [],
	...
	init: function () {
		...
		function updateClass() {
			...
			if (this._customClasses.length > 0) {
				this._element.className = this._customClasses.join(" ") + " " + str;
			} else {
				this._element.className = str;
			}
		}

		this.bind("NewComponent", updateClass).bind("RemoveComponent", updateClass).bind("_classChange", updateClass);
	...
	addClass: function(cls) {
		if (this._customClasses.indexOf(cls) == -1) {
			this._customClasses.push(cls);
			this.trigger("_classChange");
		}
	},

	removeClass: function(cls) {
		var index = this._customClasses.indexOf(cls);
		if (index != -1) {
			this._customClasses = this._customClasses.splice(index - 1, 1);
			this.trigger("_classChange");
		}
	},
...
Crafty.extend({
	...
	*/
    ...
    _mouseGrabFunctions: [],
    _tryMouseGrabFunction: function (e) {
	  if (this._mouseGrabFunctions.length == 0)
		return false;

	  return this._mouseGrabFunctions[this._mouseGrabFunctions.length - 1](e);
	},

    pushMouseGrabFunction: function (fn) {
	  this._mouseGrabFunctions.push(fn);
	},

    popMouseGrabFunction: function () {
	  this._mouseGrabFunctions.pop();
    },
	...
	*/
	mouseDispatch: function (e) {
	    ...
	    // give a grabby mouse handler a chance
	    if (Crafty._tryMouseGrabFunction(e)) {
		  return;
		}
		...
		var draw = function (e) {
			if (e.type === "canvas") {
			    ...
			    if (self._repeat == "no-repeat") {
					context.scale(this._w / this.img.width,
								  this._h / this.img.height);
				}
				context.fillRect(0, 0, this._w, this._h);
				context.restore();
			} else if (e.type === "DOM") {
				if (this.__image != this.__lastImage ||
					this._repeat != this.__lastRepeat)
				{
					e.style.background = "url(" + this.__image + ") " + this._repeat;
					if (this._repeat == "no-repeat") {
						e.style.backgroundSize = "cover";
					}
					this.__lastImage = this.__image;
					this.__lastRepeat = this._repeat;
				}
			}
		};
	...
	image: function (url, repeat) {
	    ...
	    var imageLoaded = function() {
		  ...
		  if (self.w == 0 && self.h == 0) {
			...
			self.w = Math.round(imgw * (self.h / imgh));
		  } else if (self.h == 0) {
			self.h = Math.round(imgh * (self.w / imgw));
		  }
